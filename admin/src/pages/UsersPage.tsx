...
import type { YonnmaUser } from '../lib/types';
...
export default function UsersPage() {
  const [users, setUsers] = useState<YonnmaUser[]>([]);
  ...
  return (
    <div>
      <PageHeader
        title="Utilisateurs"
        subtitle={`${users.length} compte${users.length > 1 ? 's' : ''} créé${users.length > 1 ? 's' : ''} sur Yonnma`}
      />
